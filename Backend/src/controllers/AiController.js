exports.getAIResponse = async (req, res) => {
    try {
        ...
        if (q.includes("today's sales")) {
            answer = "📊 Today's Sales Update:\n- Confirmed Revenue: $425,000\n- Total Bookings: 14 Units\n- Top Performer: The Atrium Plaza.";
        } else if (q.includes("pending bookings")) {
            answer = "⏳ Pending Bookings:\nThere are currently 3 unconfirmed bookings pending downpayment (Total: $150,000). Also, 2 bookings are awaiting KYC approval.";
        } else if (q.includes("customer follow ups")) {
            answer = "⚠️ Follow-ups Required:\n- 3 Hot leads in 'The Atrium' have reached 48hrs without contact.\n- Marcus V. needs payment plan modification.\n- 5 VIP clients pending 30-day routine check-in.";
        } else if (q.includes("revenue summary")) {
            answer = "📈 Revenue Summary:\n- Current Month: $1.2M Collected\n- Q3 Projection: $5.4M (110% of target)\n- Outstanding Payments: $1.2M\n- Conversion Rate: 8.4% (Trending Up).";
        } 
        else if (q.includes("highest intent score")) {
